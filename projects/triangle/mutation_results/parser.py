	...
	with open(inMutatantsFile, 'rb') as mutantFile:
		for line in mutantFile:
			...
			if (len(line) > 1):
				...
				info[colons[0]] = {"operator" : colons[1], "Description" : ":".join(colons[2:]), "killedBy" : []}
	...
	with open(inKilleldFile, 'rb') as csvfile:
		...
		for row in killreader:
			...
			if not firstRow:
				...
				info[row[0].split(',')[1]]["killedBy"].append(row[0].split(',')[0])
			else:
...
def writeCsv(outFile, info):
	with open(outFile, 'wb') as csvfile:
		...
		for num in range(len(info)):
			mutantWriter.writerow([(num+1), info[str(num+1)]['operator'], info[str(num+1)]['killedBy'], info[str(num+1)]['Description']])
